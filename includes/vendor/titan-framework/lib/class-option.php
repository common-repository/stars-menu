...
class TitanFrameworkOption {
	...
	protected function echoOptionHeader( $showDesc = false ) {
		...
		if ( $useCustom ) {
			self::$rowIndex++;//edit by parsaatef :: add code line
			do_action( 'tf_custom_option_header', $this );
			do_action( 'tf_custom_option_header_' . $this->getOptionNamespace(), $this , $showDesc , self::$rowIndex ); //edit by parsaatef :: add $showDesc , self::$rowIndex
			return;
		}
	...
	protected function echoOptionHeaderBare() {
		...
		if ( $useCustom ) {
			self::$rowIndex++;//edit by parsaatef :: add code line
			do_action( 'tf_custom_option_header', $this );
			do_action( 'tf_custom_option_header_' . $this->getOptionNamespace(), $this , false , self::$rowIndex ); //edit by parsaatef :: add false , self::$rowIndex
			return;
		}
