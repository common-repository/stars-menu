...
class LA_IconManager
{
    ...
    protected function rewriteCSS()
    {
        ...
        if ($file) {
            ...
            //edit by parsaatef : fix change 'icon-' replace
            $new_prefix = 'la'.md5($this->font_name).'-';

            $str = str_replace('[class^="icon-"]', '[class^="' . $new_prefix . '"]' , $file);
            $str = str_replace('[class*=" icon-"]', '[class*=" ' . $new_prefix . '"]' , $str);
            $str = str_replace(".icon-", ".{$new_prefix}" , $str);
            $str = str_replace(
                '.icon {',
